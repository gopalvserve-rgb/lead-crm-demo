# SmartCRM Demo — Node.js / PostgreSQL edition

**This is the public demo edition.** It's a 1:1 fork of the production Celeste Abode CRM with three extra layers:

1. **Demo guard** — every outbound integration (email, WhatsApp, FB, FCM, Sheet sync) is short-circuited to a no-op so prospects can click freely without sending real messages.
2. **Seeded dummy data** — 1 admin + 4 reps, 100 leads, 15 properties, 30 days of activity, sample tasks/leaves/attendance/salary slips. Run `npm run seed:demo`.
3. **Interactive walkthrough** — Shepherd.js tour auto-launches on first login.

**Live demo:** https://demo.smartcrmsolution.com — sign in with `demo@smartcrmsolution.com / demo123`.
