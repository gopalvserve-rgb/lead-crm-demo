 ...
 */
/**
 * Recent recordings feed — across ALL leads + ALL reps, used by the
 * "Call insights" view. Returns recent recordings with their AI summary
 * fields pre-joined to lead + rep names so the UI doesn't need extra
 * round-trips.
 *
 * Visibility: same as call rating report.
 *   - admin / manager → see everyone's
 *   - team_leader     → see own team
 *   - sales / employee → see only own recordings
 */
async function api_recording_recentInsights(token, opts) {
  ...
  params.push(limit);
  const sql = `
    SELECT lr.id, lr.lead_id, lr.user_id, lr.phone, lr.duration_s, lr.direction,
  ...
  try {
    ...
    return rows.map(r => {
      ...
      return {
        ...
        lead_status_name: r.lead_status_name,
        phone: r.phone,
        duration_s: r.duration_s,
