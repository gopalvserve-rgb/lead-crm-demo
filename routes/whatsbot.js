...
async function _graphPost(path, body, cfg) {
  if (demo.on) {
    demo.simulate('whatsapp_graph_post', { path, body });
    // Return shape that callers expect — message-id for sends, success for everything else.
    return { status: 200, body: { messages: [{ id: 'demo_wamid_' + Date.now() }], demo: true } };
  }
...
async function _graphGet(path, cfg) {
  if (demo.on) {
    demo.simulate('whatsapp_graph_get', { path });
