...
async function _runSheetSync(integration) {
  if (demo.on) {
    // No-op in demo mode — don't hit Google CSV endpoints, don't ingest leads.
    demo.simulate('sheet_sync', { id: integration.id, name: integration.name });
    return { imported: 0, skipped: 0, total: 0, demo: true };
  }
