...
async function _gget(url) {
  if (demo.on) {
    demo.simulate('fb_graph_get', { url: String(url).split('access_token=')[0] });
    // Empty pages list is fine — admin UI will show "no pages connected" which is realistic for a demo.
    return { data: [] };
  }
...
async function api_fb_connect(token, shortToken) {
  ...
  if (me.role !== 'admin') throw new Error('Admin only');
  if (demo.on) {
    return demo.simulate('fb_connect', { admin: me.email }, {
      ok: true, demo: true,
      message: 'Demo mode — Facebook is not actually connected. In production this links your Pages and starts pulling Lead Ads.',
      pages: []
    });
  }
