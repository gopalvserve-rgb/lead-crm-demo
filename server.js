...
// One-shot demo seed: when DEMO_MODE=1 and the leads table is empty,
...
// Boolean flag the SPA reads on every load. Lets the UI hide destructive
// buttons / show the demo banner without baking flags into the bundle.
app.get('/demo-status', (_req, res) => {
  res.json({ demo: demo.on });
});
// Authenticated peek at the in-memory simulated-sends log so admins can
// show prospects exactly what would have gone out. No PII reaches the wire
// in demo mode (the seeded users are fictional).
app.get('/api/demo-log', async (req, res) => {
  if (!demo.on) return res.status(404).json({ error: 'not in demo mode' });
  const limit = Math.min(500, Math.max(1, Number(req.query.limit) || 100));
  res.json({ log: demo.recent(limit) });
});
