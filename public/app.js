...
const CRM = {
  ...
  user: null,
  config: { company_name: 'SmartCRM Demo', company_logo_url: '', base_url: location.origin },
  cache: {},
  prefs: {
...
// Placeholder shown when a lead has no recordings yet — gives prospects a
...
function aiSummaryPlaceholder() {
  return h('li', { class: 'rec-item rec-placeholder' },
    ...
    h('div', { class: 'placeholder-foot muted' },
      '↑ This is what you will see here after the first call is recorded. Upload a recording or call from the SmartCRM dialler to generate a real AI summary.'
    )
  );
}
 ...
 */
/**
 * Call Insights — the showcase view of AI call analysis.
 * A scrollable feed of every recent call with its AI summary, action
 * items, sentiment, and suggested status — across all leads + reps.
 * Most prospects open this first to see what the AI does.
 */
VIEWS.callinsights = async (view) => {
  ...
  async function load() {
    ...
    try {
      ...
      if (!rows || rows.length === 0) {
        feedDiv.innerHTML = '<div class="muted">No call recordings yet. Once your team starts uploading recordings (or syncing from the Android app), Gemini will analyse them and the summaries will appear here.</div>';
        return;
      }
      feedDiv.innerHTML = '';
      // KPIs row at top
      ...
      feedDiv.appendChild(h('div', { class: 'cards', style: 'margin-bottom:1rem' },
        ...
        kpiCard('😊 Positive',  pos + ' (' + Math.round(pos/total*100) + '%)', 'Strong intent calls', 'ok'),
        kpiCard('😟 Negative',  neg + ' (' + Math.round(neg/total*100) + '%)', 'Need manager review', 'warn'),
  ...
  function insightCard(r) {
    ...
    const dur = Number(r.duration_s) || 0;
    const mm = Math.floor(dur / 60), ss = (dur % 60).toString().padStart(2, '0');
    const card = h('div', { class: 'card insight-card' },
      h('div', { class: 'insight-header' },
        ...
        h('div', {},
          h('span', { class: 'ai-sentiment-pill', style: 'background:' + sentColor },
            sentLabel + ' ' + (r.sentiment || '—')),
          r.ai_suggested_rating ? h('span', { class: 'ai-rating-pill', title: 'AI rating' },
            '🤖 ' + r.ai_suggested_rating + '/5') : null,
          r.rating ? h('span', { class: 'ai-rating-pill', style: 'background:#fef3c7;color:#92400e' },
            '★ ' + r.rating + '/5') : null
        )
      ),
      ...
      r.action_items && r.action_items.length > 0
        ? h('div', { class: 'insight-actions' },
            h('span', { class: 'insight-actions-title' }, '✓ Next steps:'),
            ' ',
            r.action_items.slice(0, 3).join(' · ')
          )
        : null,
      r.suggested_status_name && r.suggested_status_name !== r.lead_status_name
        ? h('div', { class: 'insight-suggest' },
            '🎯 AI suggests status: ',
            h('b', {}, r.suggested_status_name),
  ...
  view.appendChild(h('div', { class: 'card' },
    ...
    h('p', { class: 'muted' }, 'Each call recording is automatically transcribed + summarised by AI. See the full conversation transcript inside any lead\'s recording row. Click on any lead below to see the full AI summary.')
  ));
  view.appendChild(h('div', { class: 'toolbar' },
...
/**
 * AI usage view (client-facing). Shows ONLY the price the client pays —
 * never the vendor cost or markup percentage. The platform owner sees
 * vendor cost via the raw API response (`cost_inr_at_cost`) outside this
 * UI, or via SHOW_VENDOR_COST=1 env (not implemented in UI to keep
 * client tenants clean).
 */
VIEWS.aiusage = async (view) => {
  ...
  try {
    ...
    // KPI cards — only "Cost", "Calls", "Forecast", "All-time"
    ...
    // This month breakdown — billable only
